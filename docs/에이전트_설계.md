...
원문 ④ Usage Prediction / Segmentation은 **실서비스 경로에 없다.** 검증 실험
(`segment_ml_check.ipynb`)에서만 쓴다. KMeans로 세그먼트를 만들고 그 세그먼트의
요금제를 주면, 합성 고객 1,435명 중 예산 조건의 81.7%, 데이터 조건의 72.3%를
어기기 때문이다(군집 축을 7개로 늘려도 93.1% → 98.3%로 나빠진다). 원문 ⑦
Evaluation도 온라인이 아니라 같은 노트북이 대신한다.
...
**로직을 노드 안에 넣지 않는다.** 웹(`src/api.py`)·노트북(`segment_ml_check.ipynb`)·
...
api.py             엔드포인트 응답 형태
make_synthetic_mvno.py  합성 데이터 자기정합성
```
