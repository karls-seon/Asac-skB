...
넘으면 "조건에 맞는 요금제가 없다"고 정직하게 반환한다(합성 데이터의
"예산하한-최저가미만" 엣지 케이스가 정확히 이 경로를 테스트하기 위한 것).
...
정도 쓰고 싶어요")에서 구조화된 `user_profile`을 뽑는다. 뽑을 슬롯은 이미
합성 데이터 스키마로 초안이 나와 있다(`synthetic_customers.csv` +
`synthetic_preferences.csv`) - `age`, `monthly_call_minutes`,
`monthly_data_gb`, `monthly_sms_count`, `monthly_budget_won`(선택 - 없으면
Plan Matching이 가격 랭킹만으로 정렬), `ott_preference`,
`preferred_carrier_type`, `data_unlimited_required`/`voice_unlimited_required`/
`sms_unlimited_required`, `contract_discount_ok`.
...
1. **세그먼트 배정(대부분의 경우)**: User Profiling이 이미 실제 사용량 숫자를
   받아왔으면, 그 숫자가 `USAGE_SEGMENTS`(S1~S6) 중 어느 구간에 드는지 규칙
   기반으로 바로 매칭한다 - ML이 필요 없다.
...
| 2 | Usage Prediction / Segmentation | 사용자를 이용형태 세그먼트로 분류, 다음달 사용량 예측(회귀) | `user_profile` | `predicted_segment`(str), `predicted_usage`(dict) | 미구현 (합성 고객 데이터에 K-means/GMM 적용 예정) |
...
- User Profiling Agent를 제일 먼저 만드는 걸 추천한다 - 여기서 정하는
  ...
  기반이라, 먼저 확정해야 뒤가 안 흔들린다. 합성 데이터의
  `synthetic_customers.csv`/`synthetic_preferences.csv` 컬럼 구성이 이미
  이 스키마의 초안 역할을 한다(같은 조직이 팀원 작업으로 넘어갔으니, 합쳐질 때
  이 스키마도 같이 맞추면 된다).
