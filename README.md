...
│   │  --- 실험 ---
│   └── make_synthetic_mvno.py  알뜰폰 합성 고객 4만 (세그먼트 검증용)
...
│   ├── 합성데이터_생성근거.md   합성 고객을 만든 근거 — 출처·가정·검증·한계
│   └── 수정이력.md           고친 데이터 오류와 그 이유
└── segment_ml_check.ipynb  세그먼트 분류 ML이 필요한지 검증 (KMeans vs 필터+랭킹)
```
...
3. **`data/external/`(KISDI 원자료)도 git에 없다.** 한국미디어패널조사에 신청해서
   받은 자료라 재배포하지 않는다. 다만 **지금은 없어도 된다** — 합성 고객이 쓰는
   연령 분포는 `make_synthetic_mvno.py`에 상수로 박혀 있고, 유도 과정과 재현
   쿼리를 주석에 남겨 뒀다. 원자료로 그 표를 다시 뽑고 싶을 때만 필요하다.
4. **`data/synthetic/`도 git에 없다.** 고정 시드라
   `python src/make_synthetic_mvno.py`로 같은 파일이 다시 만들어진다.
...
고객을 KMeans로 나누고 **그 세그먼트가 쓰는 요금제**를 주는 방식이 흔한 대안입니다.
그쪽을 실제로 만들어 비교했습니다([segment_ml_check.ipynb](segment_ml_check.ipynb)).

추천에는 정답이 없어 "정확도"를 잴 수 없습니다. 대신 **사용자가 말한 조건을 지키는지**를
알뜰폰 합성 고객 1,435명으로 쟀습니다.
...
군집 축을 2개에서 7개로 늘려도 나아지지 않습니다. 원인은 축 선택이 아니라 구조입니다 —
세그먼트 방식은 사용자의 예산을 **보지 않고** 덩어리 번호만 보고 답합니다. 한 덩어리
안에서 예산이 900원~53,000원까지 벌어지는데 거기에 요금제 묶음 하나를 주면 낮은 쪽은
반드시 초과합니다. K를 키우면 줄지만, 끝까지 키우면 그건 사람마다 따로 거르는 것 —
필터입니다.

> 시험용 고객이 훈련용과 같은 합성 분포에서 나와 **세그먼트 방식에 유리하게 기운**
> 비교인데도 이렇습니다. 알뜰폰 전용 데이터라 통신사 위반 축은 구조적으로 0이어서
> 표에서 뺐습니다.
