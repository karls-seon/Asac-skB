...
규칙:
- 사용자가 말하지 않은 슬롯은 **키 자체를 넣지 마라**. false나 null을 넣는
  것도 안 된다("무제한 필요 없다"는 말은 data_unlimited_required=false가
  아니라 그 키를 생략하는 것이다).
- **숫자를 말했으면 반드시 넣어라.** "데이터 200기가"처럼 명시했는데
  data_usage_gb를 빼먹으면 안 된다.
- ott_preference는 **요금제에 그 구독이 끼어 있길 원할 때만** 넣어라.
  "유튜브를 자주 본다 / 넷플릭스를 많이 본다"는 데이터를 얼마나 쓰는지에
  대한 설명이지 구독 선호가 아니다 - 이때는 data_usage_gb만 추정하고
  ott_preference는 비워라. "넷플릭스가 포함된 / 넷플릭스 되는 요금제"처럼
  요금제에 붙길 원한다고 해야 넣는다.
