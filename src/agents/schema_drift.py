...
2026-08-03에 KT가 요금표를 외부 JS 파일로 옮기면서 주력 4개 상품이 통째로 0건이
됐는데 **에러가 하나도 안 났다** - HTTP 200, 파싱 예외 없음, 종료코드 0
(docs/수정이력.md 35번). 원인 추적 시간의 대부분이 "무엇이 어디로 옮겨갔는지"
원본 HTML을 뒤지는 데 들어서, 그 부분을 자동화한다.

## 어떻게 찾는가 - 어제는 되던 소스가 오늘 0행이면 의심한다

캐시 파일 하나가 파싱 결과 하나에 대응한다(detail_23450.html -> plan_id 23450).
여기서 **"캐시에 있는데 CSV에 없는 파일"을 그냥 세면 안 된다** - 수집 범위 밖이라
일부러 안 쓰는 캐시가 많아서(SKT는 147개를 받아 50개만 쓴다) 신호가 잡음에 묻힌다.
그래서 `parse_coverage.json`에 남긴 **직전에 성공했던 커버리지와 비교**한다.
범위 밖 캐시는 기준선에도 없으므로 조용하다.

회귀 파일에서 **파서가 의존하는 앵커**(KT의 `tableHTML` 등)가 아직 있는지까지
확인해 리포트에 적는다. 앵커가 사라졌으면 그 자리가 바뀌었다는 강한 단서다.

## 하지 않는 것

**파서 코드를 고치지 않는다.** 이 프로젝트에서 이미 두 번, 몇 개 사례만 보고
일반화한 수정이 새 오류를 만들었다(수정이력 34번, 15번). 자동 수정은 같은 실수를
더 빠르고 조용하게 저지른다.
"""
import csv
import json
...
# 리포트에 실을 예시 개수. 원인은 대개 몇 개만 열어봐도 같다.
MAX_SAMPLES = 15
...
# plan_id가 "{소스코드}_{접미사}" 꼴이라(접미사는 연령/택1 전개용) 첫 '_' 앞이
# 캐시 파일의 소스 키다. 모요는 숫자 id라 '_'가 없다.
...
def diagnose(sites: list[str] | None = None, run_date: str | None = None,
    ...
    회귀가 있을 때는 **기준선을 갱신하지 않는다.** 갱신하면 망가진 상태가
    "정상"이 돼서 같은 문제를 다시는 못 잡는다.
    """
