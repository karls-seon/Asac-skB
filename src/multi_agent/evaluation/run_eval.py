...
비교한다. **그래프 노드가 아니다** - 사용자 요청마다 도는 게 아니라 오프라인 배치다.

    cd src && python -m multi_agent.evaluation.run_eval
...
- median_recommended_fee: 추천 요금 중앙값

## 입력은 격자로 만든다

예산 x 데이터 x mvno_ok x 통화 격자. 합성 고객(`src/make_synthetic_mvno.py`)을
재사용하면 세그먼트 방식이 자기가 학습한 분포에서 시험 보는 셈이라 비교가 기운다.
...
def make_grid() -> list[dict]:
    """평가 입력 격자. 예산 x 데이터 x mvno_ok x 통화."""
