...
## carrier_type 주의사항
carrier_type은 "어느 채널에서 수집했나"를 뜻한다. 통신사 사이트에서 직접 수집한
요금제가 MNO, 모요(plan_category="moyo")에서 수집한 요금제가 MVNO다.
너겟(LGU+)/다이렉트(SKT)/요고(KT)는 통신사 자사 온라인전용 요금제지만 모요에도
올라와 있어서, 모요판은 MVNO로 통신사 사이트판은 MNO로 **두 행이 모두 존재**한다
(2026-08-13 기준 45건, 이 중 42건이 이름상 중복). 요금제 개수·평균가를 셀 때는
이 중복을 감안해야 한다. mvno_brand에 "KT"/"SKT"/"LG U+"가 들어있으면 모요판
통신사 요금제라는 뜻이다.
